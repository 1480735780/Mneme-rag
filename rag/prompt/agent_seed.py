# -*- coding: utf-8 -*-
"""
内置智能体与提示词槽位种子数据（对齐 ragent-new resources/database 种子 SQL）

Java 侧由 init_data_pg.sql（内置智能体档案）+ 260812_agent_engine.sql（AGENT_MAIN /
KNOWLEDGE_TOOL_DESCRIPTION 两槽位）在迁移时播种；Python 侧由 wiring 的
`_ensure_seed_agent_prompt` 幂等播种（进程启动时缺行补行，已有非空内容不覆盖）。

内置智能体是所有空槽位的回落终点（DatabaseAgentPromptResolver._load_from_db 的
builtin 基线）；AGENT_MAIN 人设没有代码内置默认（DEFAULT_AGENT_PROMPTS 仅收运营槽位），
缺种子即 Agent 引擎 fail-fast（"Agent人设内容不允许为空"）。
"""
from __future__ import annotations

# 内置智能体档案（对齐 Java init_data_pg.sql：不可编辑、不可删除，空槽位回落终点）
BUILTIN_AGENT_ID = "2001523723396309001"
BUILTIN_AGENT_PROFILE = {
    "id": BUILTIN_AGENT_ID,
    "name": "默认助手",
    "description": "系统默认人设，其他智能体没自定义的提示词都沿用这里的内容",
    "avatar": "orbit-indigo",
    "builtin": 1,
    "active": 1,
}

# AGENT_MAIN：主 Agent 系统提示词（对齐 Java 260812_agent_engine.sql，逐字移植）
AGENT_MAIN_PROMPT = """# 身份
你是一个能调用工具完成任务的智能助手。
...
# KNOWLEDGE_TOOL_DESCRIPTION：search_knowledge 工具描述（主 Agent 据此判断是否调用）
KNOWLEDGE_TOOL_DESCRIPTION_PROMPT = """检索本助手配置的知识库，返回一份已基于命中资料合成的完整答案。
...
BUILTIN_AGENT_PROMPT_SEEDS = {
    ...
    "KNOWLEDGE_TOOL_DESCRIPTION": KNOWLEDGE_TOOL_DESCRIPTION_PROMPT,
}
