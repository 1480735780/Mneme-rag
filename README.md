...
[![Tests](https://img.shields.io/badge/Tests-683%20passed-brightgreen?style=flat-square)](https://github.com/your-org/mneme-rag/actions)
...
- **混合检索**：向量、关键词、知识图谱（LightRAG + Neo4j）、联网搜索四通道并行召回，支持去重、RRF 融合与 Rerank。
- **问题理解**：查询词映射、问题重写与拆分、LLM 树形意图识别与向量意图分类，多知识库路由。
- **模型路由**：多供应商候选（Qwen / OpenAI / Ollama / SiliconFlow / AIHubMix），档位解析、首包探测、故障转移、熔断降级。
- **MCP 工具**：独立 MCP Server（Weather / Sales / Ticket / You.com Search）+ Streamable HTTP 客户端自动发现。
- **会话记忆**：最近 N 轮消息 + 持久化摘要，控制 Token 成本并保留关键上下文。
- **流量保护**：Redis 公平排队 + 分布式并发控制 + 幂等提交防重。
- **管理后台**：React 18 + TypeScript 全功能前端——聊天、知识库、Trace、Dashboard、用户、审计、意图树、Agent、Pipeline、图谱。
...
| 后处理链 | 去重 → RRF 融合 → Channel 归因 → Metadata 补全 → Rerank |
...
| 幂等提交 | 提交防重（setnx）+ 消费防重（key timeout） |
...
│   ├── memory/            # 会话记忆 + 摘要
│   ├── prompt/            # Prompt 模板（.st）
│   └── service/           # Chat Service / Stream Handler / Trace / Rate Limit
...
| 测试 | pytest (683 tests), Vitest (214 tests), Playwright (E2E) |
